...
import { fetchCoauthorshipNetwork } from "../api/graphApi";
import type {
  CytoscapeElement,
  GraphEdge,
  GraphNode,
...
interface UseProgressiveCoauthorshipReturn {
  ...
  initGraph: (authorId: string) => void;
  seedId: string | null;
  reset: () => void;
}
...
export function useProgressiveCoauthorship(): UseProgressiveCoauthorshipReturn {
  ...
  const initGraph = useCallback((authorId: string) => {
    initAbortRef.current?.abort();
    expandAbortRef.current?.abort();
    ...
    fetchCoauthorshipNetwork(authorId, 1, 15, controller.signal)
      .then((response) => {
        if (controller.signal.aborted) return;
        setSeedId(authorId);
        setNodesMap(mergeNodes(new Map(), response, authorId));
      ...
      .catch((err: unknown) => {
        if (!controller.signal.aborted) {
          const message =
            err instanceof Error ? err.message : "Failed to load co-authorship network";
          setError(message);
          setLoading(false);
        }
      });
  ...
  const expandNode = useCallback(
    (nodeId: string) => {
      ...
      fetchCoauthorshipNetwork(nodeId, 1, 10, controller.signal)
        .then((response) => {
          if (controller.signal.aborted) return;
