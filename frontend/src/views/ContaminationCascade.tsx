...
import { useProgressiveCascade } from "../hooks/useProgressiveCascade";
...
export function ContaminationCascade() {
  ...
  useEffect(() => {
    ...
    if (urlSeed && !seedId) {
      ...
      // "Primary Prevention of Cardiovascular Disease with a Mediterranean Diet" (retracted, 4187 cit)
      const defaultSeed = "W2166000498";
  ...
  const contextMenuActions = useMemo<ContextMenuAction[]>(() => {
    ...
    return [
      {
        label: expanded ? "Already Expanded" : "Expand Cascade",
        icon: (
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 px-3 sm:px-5 py-2 sm:py-3 border-b border-[#2a2a4a] bg-[#1a1a2e]/50">
        ...
        <SearchBar
          searchType="papers"
          placeholder="Search for a retracted paper..."
      ...
      <GraphControls
        ...
        edgeCount={edgeCount}
      >
        <ExportMenu cyRef={cyRef} elements={filteredElements} filenamePrefix="retraction_cascade" />
      </GraphControls>
      ...
      <div className="flex flex-1 overflow-hidden">
        {!seedId && !loading && !error && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center max-w-md">
              <div className="w-12 h-12 rounded-full bg-[#ff4757]/10 flex items-center justify-center mx-auto mb-3">
                <svg className="w-6 h-6 text-[#ff4757]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <title>Retraction cascade icon</title>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                </svg>
              </div>
              <p className="font-[Outfit] text-gray-500">
                Search for a retracted paper to trace how its citations
        ...
        {loading && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <div className="w-8 h-8 border-2 border-[#ff4757]/30 border-t-[#ff4757] rounded-full animate-spin mx-auto" />
              <p className="font-[Outfit] text-sm text-gray-500 mt-3">
                Tracing contamination cascade...
              </p>
            </div>
          </div>
        )}
        ...
        {!loading && !error && seedId && filteredElements.length === 0 && (
          <EmptyGraphState seedId={seedId} message="No contamination cascade found for this paper. It may not be retracted or have no citing papers." />
        )}
